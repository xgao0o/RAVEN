...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.use_rope:
            if RotaryEmbedding is None:
                raise ImportError("rotary_embedding_torch not found, but rotary=True was set.")
    ...
    def forward(self, x, days: Optional[torch.Tensor] = None, kv_state: Optional[KVState] = None):
        ...
        if self.flash:
            # Flash SDPA path.
            # For KV cached decode (T may be 1, Tp >= T), some builds mishandle boolean masks.
            # Use a fully-shaped *additive* mask (float, -inf where masked) for the KV case.
...
# ----------------------------
# Model (in-place KV-cache capable)
# ----------------------------
class GPT(nn.Module):
    ...
    # ----- Multiset loss (parallel; unordered intra-visit supervision) -----
    def _multiset_loss_parallel(
        self,
        logits: torch.Tensor,            # (B, T, V)
        idx: torch.Tensor,               # (B, T)
        remaining_indices: torch.Tensor, # (B, T, Kmax) -1 padded
        need_sep_mask: torch.Tensor,     # (B, T) bool
        supervise_mask: torch.Tensor = None  # (B, T) bool; True where we supervise
    ) -> torch.Tensor:
        cfg = self.config
        logp = torch.log_softmax(logits, dim=-1)
        rem = remaining_indices.to(logits.device)
        need_sep = need_sep_mask.to(logits.device).bool()
        tokens = idx.to(logits.device)
        default_valid = (tokens != cfg.pad_id) & (tokens != cfg.eow_id) & (tokens != cfg.sep_id)
        valid = supervise_mask.to(logits.device).bool() if supervise_mask is not None else default_valid
        safe  = rem.clamp_min(0)
        gmask = (rem >= 0)
        rem_logp = logp.gather(-1, safe).masked_fill(~gmask, 0.0)
        k = gmask.sum(-1)
        loss_mt = torch.zeros_like(logp[..., 0])
        nonempty = (k > 0)
        if nonempty.any():
            sumlog = rem_logp.sum(-1)
            loss_mt[nonempty] = -(sumlog[nonempty] / k[nonempty].to(logp.dtype))
        sep_logp = logp[..., cfg.sep_id]
        loss_mt[need_sep] = -sep_logp[need_sep]
        loss_mt = loss_mt * valid.to(loss_mt.dtype)
        denom = valid.sum().clamp_min(1)
        return loss_mt.sum() / denom
    ...
    def forward(
        self,
        idx: torch.Tensor,                          # (B, T)
        targets: Optional[torch.Tensor] = None,     # (B, T) or None (CE mode)
        ...
        # multiset loss (parallel) inputs:
        remaining_indices: Optional[torch.Tensor] = None,  # (B,T,Kmax) -1 padded
        need_sep_mask: Optional[torch.Tensor] = None,      # (B,T) bool
        supervise_mask: Optional[torch.Tensor] = None,
        loss_mode: str = "ce"                         # "ce" | "multiset"
    ):
        device = idx.device
        B, T = idx.shape
        ...
        elif loss_mode == "multiset":
            assert remaining_indices is not None and need_sep_mask is not None, \
                "Multiset loss requires `remaining_indices` and `need_sep_mask`."
            loss = self._multiset_loss_parallel(
                logits, idx, remaining_indices, need_sep_mask, supervise_mask
            )
        return logits, loss
    ...
    # ----- Simple generation (unchanged API) -----
    @torch.no_grad()
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        config_args = config_args[model_type]
