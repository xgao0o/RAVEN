# train.py
# Multiset-loss training script (DDP-friendly)
# - Causal attention; optional RoPE/XPos
# - Uses UnifiedSeqEHRDataset with optional multiset targets
# - Default: parallel multiset loss (unordered, per-visit supervision)
# - NO validation loop; logs train loss only; saves checkpoints at save_interval
...
# wandb
wandb_log = True
wandb_project = "gpt-ntp-ehr"
wandb_run_name = "run-multiset"

# data / loader
gradient_accumulation_steps = 8     # effective batch size = this * batch_size
batch_size = 8
block_size = 512
num_workers = 0          # IMPORTANT for shelve safety
pin_memory = True
multiwin = True          # use multi-patient training windows
shuffle_within_groups = True

# model
n_tokens = 42337                    # base code vocab (without specials)
special_tokens = 6                  # [PAD, SEP, CLS, MASK, UNK, EOW]
vocab_size = n_tokens + special_tokens
n_layer = 8
n_head = 8
n_embd = 1024
dropout = 0.0
...
# loss mode (default to multiset)
loss_mode = "multiset"             # "multiset" | "ce"
build_multiset_targets = (loss_mode == "multiset")
...
train_dataset = UnifiedSeqEHRDataset(
    ...
    train=True,
    multiwin=multiwin,
    shuffle_within_groups=shuffle_within_groups,
    build_multiset_targets=build_multiset_targets
)
...
def load_latest_checkpoint(dirname, device_):
    ...
    path = ckpts[-1]
    print("Loading", path)
    return torch.load(path, map_location=device_)
...
while True:
    ...
    for batch in train_loader:
        ...
        days    = batch["days_embed_arr"].to(device, non_blocking=True)
        pids    = batch.get("patient_ids")
        pids    = pids.to(device, non_blocking=True) if pids is not None else None

        # optional CE targets (only used if loss_mode == "ce")
        targets = batch.get("targets")
        targets = targets.to(device, non_blocking=True) if targets is not None else None

        # multiset tensors (required if loss_mode == "multiset")
        rem      = batch.get("remaining_indices")
        need_sep = batch.get("need_sep_mask")
        sup      = batch.get("supervise_mask")

        # forward + loss (with grad accumulation)
        with ctx:
            if loss_mode == "multiset":
                assert rem is not None and need_sep is not None, \
                    "Multiset loss selected but dataset did not provide remaining_indices / need_sep_mask."
                rem = rem.to(device, non_blocking=True)
                need_sep = need_sep.to(device, non_blocking=True).bool()
                logits, loss = model(
                    tokens,
                    targets=None,
                    days=days,
                    patient_ids=pids,
                    remaining_indices=rem.to(device, non_blocking=True),
                    need_sep_mask=need_sep.to(device, non_blocking=True).bool(),
                    supervise_mask=(sup.to(device, non_blocking=True).bool() if sup is not None else None),
                    loss_mode="multiset"
                )
            else:
                logits, loss = model(
        ...
        if (iter_num > 0) and (iter_num % save_interval == 0) and master_process:
            ...
            checkpoint = {
                ...
                "iter_num": iter_num,
                "best_val_loss": None,  # not used (no validation)
                "config": config,
            }
